...
class WifiManager:
  ...
  def activate_connection(self, ssid: str, block: bool = False):
    def worker():
      ...
      if conn_path is not None:
        ...
        self._connecting_to_ssid = ssid
        call = new_method_call(self._nm, 'ActivateConnection', 'ooo', (conn_path, self._wifi_device, "/"))
        if block:
          reply = self._router_main.send_and_get_reply(call)
          if reply.header.message_type == MessageType.error:
            cloudlog.warning(f"Failed to activate {ssid}: {reply}")
            return False
          return True
        self._router_main.send(call)
        return True
      return False
  ...
  def _wait_for_tethering_state(self, active: bool, timeout: float = 15.0) -> bool:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
      try:
        if self.is_tethering_active() == active:
          return True
      except Exception:
        pass
      time.sleep(0.25)
    try:
      return self.is_tethering_active() == active
    except Exception:
      return False
  ...
  def set_tethering_active(self, active: bool):
    def worker():
      ...
      if active:
        ...
        if not self.activate_connection(self._tethering_ssid, block=True):
          self._tethering_transition = False
          return
        if not self._wait_for_tethering_state(True):
          cloudlog.warning("Hotspot did not become active before timeout")
          self._tethering_transition = False
          return
        ...
        self._wait_for_tethering_state(False, timeout=10.0)
      self._tethering_transition = False
