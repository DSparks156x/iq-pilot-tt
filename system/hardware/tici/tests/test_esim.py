...
class TestEsimIsdrRecovery:
  def test_open_ready_client_recovers_isdr_with_uim_reset(self, monkeypatch):
    lpa = object.__new__(lpa_module.TiciLPA)
    lpa.device = "/dev/modem_at0"
    lpa.baud = 9600
    lpa.timeout = 5.0
    lpa.verbose = False
    lpa._last_good_device = "/dev/modem_at0"
    lpa._euicc_present_cache = True
    lpa._client = None
    ...
    monkeypatch.setattr(lpa, "_probe_euicc_present", lambda: True)
    monkeypatch.setattr(lpa, "_candidate_devices", lambda: ["/dev/modem_at0"])
    monkeypatch.setattr(lpa, "_is_modemmanager_running", lambda: True)
    monkeypatch.setattr(lpa, "_set_modemmanager_running", lambda running: modemmanager_states.append(running))
    monkeypatch.setattr(lpa_module.time, "sleep", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(lpa_module.subprocess, "check_call", lambda cmd: qmi_calls.append(cmd))

    class FakeClient:
      def __init__(self, device, baud, timeout, verbose):
        self.device = device
        self.channel = None
    ...
    monkeypatch.setattr(lpa_module, "AtClient", FakeClient)

    client = lpa._open_ready_client()
    ...
    assert modemmanager_states == [False, True]
