...
Ported from selfdrive/ui/iqpilot/layouts/settings/osm.py.
Adapted for the 536×240 display.
"""
import json
import threading
...
from openpilot.selfdrive.ui.mici.widgets.dialog import BigMultiOptionDialog, BigConfirmationDialogV2
...
try:
  from openpilot.selfdrive.ui.iqpilot.layouts.settings.osm import (
    fetch_available_regions, get_downloaded_maps, delete_maps, start_map_download,
    get_download_progress, get_mapd_version,
  )
  _OSM_AVAILABLE = True
except ImportError:
  ...
  def start_map_download(region: str):
    pass

  def get_download_progress() -> float:
    return -1.0

  def get_mapd_version() -> str:
    return "unknown"


class OSMLayoutMici(NavWidget):
  """
  Offline map download/delete management for MICI.
  Compact layout for 536×240.
  """
  ...
  def __init__(self, back_callback: Callable | None = None):
    ...
    self._regions: dict = {}         # country → list of states
  ...
  def _open_region_dialog(self):
    ...
    def on_confirm():
      ...
      # If country has sub-regions (states), open state picker
  ...
  def _start_download(self):
    ...
    threading.Thread(
      target=lambda: start_map_download(self._selected_region),
      daemon=True,
    ).start()
  ...
  def show_event(self):
    ...
    # Re-enable download button if not actively downloading
  ...
  def _render(self, rect: rl.Rectangle):
    ...
    if 0.0 <= progress < 1.0:
      ...
      if fill_w > 0:
        fill_rect = rl.Rectangle(px, prog_y, fill_w, self.PROGRESS_H)
        rl.draw_rectangle_rounded(fill_rect, 0.5, 6, rl.Color(0x00, 0xC0, 0xB0, 220))
      pct_text = f"{int(progress * 100)}%"
      rl.draw_text_ex(
        gui_app.font(FontWeight.ROMAN), pct_text,
        rl.Vector2(px + w / 2 - 20, prog_y - 1),
        22, 0, rl.Color(255, 255, 255, 180),
      )
    ...
    # Buttons below info + progress bar
